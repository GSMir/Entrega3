...
3)  restbp vs age
4)  chol vs age
5)  expeak vs restbp (POR SEXOS para ver difeerncias)
6)  Comentar la NO CORR entre Chol y MaxHR
...
cor.test(heartdisease$maxHR, heartdisease$age, alternative = "less")$p.value

```

Confirmamos que en la *población* de individuos con síntomas de dolor en el pecho (a la que, a partir de ahora nos referiremos como población "grande"), no en la muestral, la edad y el número máximo de pulsaciones tienen correlación negativa dado que se ha obtenido un pvalor inferior a `cor.test(heartdisease$maxHR, heartdisease$age, alternative = "less")$p.value`. Esto significa que, a mayor edad se espera que disminuya el número máximo de pulsaciones por minuto.
