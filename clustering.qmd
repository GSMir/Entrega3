...
fviz_dend(x = hc_centroid, k = 2, cex = 0.6)
```
...
```{r}
dend_1 <- datos_2 %>% dist(method = "euclidean") %>% hclust(method = "average") %>%  as.dendrogram()
dend_2 <- datos_2 %>% dist(method = "euclidean") %>% hclust(method = "single") %>%  as.dendrogram()
dend_3 <- datos_2 %>% dist(method = "euclidean") %>% hclust(method = "complete") %>%  as.dendrogram()
dend_4 <- datos_2 %>% dist(method = "euclidean") %>% hclust(method = "median") %>%  as.dendrogram()
dend_5 <- datos_2 %>% dist(method = "euclidean") %>% hclust(method = "centroid") %>%  as.dendrogram()

list_dendrogramas <- dendlist("average" = dend_1, "single" = dend_2,
 "complete" = dend_3, "median" = dend_4,"centroid"=dend_5)
